...
export function palindromeRearranging(inputString: string): boolean {
    ...
    let charMap = {}
    for (
        let i = 0; i < inputString.length; i++) {
        !charMap[inputString[i]] ? charMap[inputString[i]] = 1 : charMap[inputString[i]]++;
    }

    /* Check through each character in thr map object. In order to be arranged into a palindrome,
      only one set of characters can be odd (the center letter).
    */
    let oddsLeft = 1;
